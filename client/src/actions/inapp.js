...
import { inapp as newInapp, type as newType, button as newButton } from '../shared/constants';
...
export const loadInapp = (id) => async dispatch => {
  ...
  if (id === undefined) {
    dispatch({
      type: LOAD_INAPP,
      payload: { currentInapp: newInapp, type: newType, buttons: buttons }
    })
  } else {
    let headers = {
    ...
    res.json()
      ...
      .catch(err => dispatch(setAlert('Could not fetch the requested Inapp', 'danger')));
  }
}
